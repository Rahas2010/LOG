  ...
  return (
    <div className={`${isDark ? 'dark-card' : 'glass-card'} p-6 md:p-8`}>
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <div>
          <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Behavior Score</h3>
          <p className={`text-xs font-medium ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Based on last 7 days</p>
        </div>
      </div>
      ...
      <div className="flex flex-col items-center">
        <div className="relative w-44 h-44 mb-5">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className={`text-[10px] font-semibold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>/ 100</span>
          </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <span className={`text-xl ${isDark ? 'text-gray-600' : 'text-gray-300'}`}>•</span>
          <span className={`text-base font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>{label}</span>
        </motion.div>

        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          transition={{ delay: 0.6 }}
          className={`w-full mt-6 pt-5 border-t space-y-3 ${isDark ? 'border-[#2a2a3e]' : 'border-gray-200/50'}`}>
          <div className="flex items-center justify-between">
            <span className={`text-sm flex items-center gap-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              <div className="w-6 h-6 rounded-lg bg-green-100 flex items-center justify-center">
                <Target className="w-3 h-3 text-green-600" />
              </div>
              On-time rate
            </span>
            <span className={`text-sm font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>{onTimePct}%</span>
          </div>
          <div className="flex items-center justify-between">
            <span className={`text-sm flex items-center gap-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            ...
            <span className={`text-sm font-bold ${avgPenalty > 0 ? 'text-red-500' : (isDark ? 'text-white' : 'text-gray-900')}`}>
              {avgPenalty} min
            </span>
          </div>
        </motion.div>

        <div className={`w-full mt-5 pt-4 border-t ${isDark ? 'border-[#2a2a3e]' : 'border-gray-200/50'}`}>
